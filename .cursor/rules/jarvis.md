...
19. **Every new requirement/PR MUST start with a new Cursor chat. Your very first action is to run `start_pr_session.py` and surface the deeplink.** When the user says anything like "let's work on X", "new requirement", "start PR N", "next task", or describes a new change — BEFORE writing a single line of code or doing any exploration, run:
    ```bash
    python3 scripts/start_pr_session.py --pr <next-n> --requirement "<requirement text>"
    ```
    Then immediately output the `cursor://` deeplink prominently and tell the user: **"Click this link to open a new IDE chat for this PR — then paste this brief there."** Do NOT continue building in the current chat after handing off the deeplink. Rationale: reusing the current chat drags the entire conversation history as cache-read tokens into every new turn ($0.50/M on Opus). A fresh chat resets the counter, saving ~30% of build cost. The platform limitation is real — you cannot open a new tab yourself; the one-click deeplink is the lightest possible ask. Concrete miss to prevent: continuing to implement a new requirement in the same chat as the previous PR's discussion, as happened on PR #15 (90% cache-read tokens, ~$3.76 avoidable).
