...
19. **Every new requirement MUST start in an isolated worktree + new Cursor chat — do NOT implement in the current chat.** When the user says anything like "let's work on X", "new requirement", "next task", or describes a new change — your **very first action** (before any code, exploration, or branch checkout in the shared tree) is:
    ...
    **NEVER assume or guess a PR number when starting a session.** GitHub assigns the PR number only when `gh pr create` actually runs. The session is tracked **provisionally by its branch** (`metrics/pr_cost/session-<slug>.json`). **Right after you open the PR** in the worktree:
    ...
    ```
    Concrete misses to prevent: (a) implementing a new requirement in the same chat as the previous PR (cache-read bloat, PR #15); (b) checking out a new branch in the **shared** tree while other chats are working (cross-contamination — PR #23/#24 cost ledger); (c) assuming PR #23 when GitHub assigned #24; (d) opening the deeplink before opening the worktree folder in Cursor (chat lands in the wrong window).
