...
19. **Every new requirement MUST start in an isolated worktree + new Cursor chat — do NOT implement in the current chat.** When the user says anything like "let's work on X", "new requirement", "next task", or describes a new change — your **very first action** (before any code, exploration, or branch checkout in the shared tree) is:
    ```bash
    python3 scripts/new_requirement.py --requirement "<requirement text>" [--branch fix/<slug>] [--requirement-id N]
    ```
    That single command: (1) creates a **sibling git worktree** on a fresh branch off `origin/main` so concurrent chat spaces never fight over one checkout; (2) runs `start_pr_session.py` there (branch-keyed cost session — **no assumed PR number**); (3) opens **Cursor in a new window** on the worktree and seeds a fresh **Agent chat on Sonnet 4.6 medium thinking** via `cursor://` deeplink (`model=claude-4.6-sonnet-medium-thinking`). **Stop in this chat after handoff** — do not continue building here.

    Fallback (worktree already exists): `cd <worktree>` then `python3 scripts/start_pr_session.py --branch <branch> --requirement "…" --open-cursor`.

    **NEVER assume or guess a PR number when starting a session.** GitHub assigns the PR number only when `gh pr create` actually runs. The session is tracked **provisionally by its branch** (`metrics/pr_cost/session-<slug>.json`). **Right after you open the PR** in the worktree:
    ```bash
    ...
    ```
    Concrete misses to prevent: (a) implementing a new requirement in the same chat as the previous PR (90% cache-read tokens, ~$3.76 avoidable — PR #15); (b) checking out a new branch in the **shared** tree while other chats are working (cross-contamination — PR #23/#24 cost ledger); (c) assuming PR #23 when GitHub assigned #24 — cost gate failed on missing `PR-24.json` (fixed by branch-keyed provisional sessions + `bind-pr`); (d) opening the deeplink before opening the worktree folder in Cursor (chat lands in wrong window); (e) **branch_window attribution across overlapping chats double-counted ~$16 in the team report** (2026-06-04 — fixed by conversation-scoped sync + `dedup-sessions` + merged-only `report.html`).
