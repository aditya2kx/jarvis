...
20. **PR descriptions MUST use the 5-section template verbatim — CI fails otherwise.** Never pass `--body` with a free-form string to `gh pr create`. Always use `gh pr create --base main --fill` and fill in the opened template. The `PR Description` CI check (`scripts/check_pr_description.py`) will fail on PRs where any of the 5 sections (§1 What is the change, §2 Motivation, §3 End-to-end test, §4 Backward compatibility, §5 Checklist) are missing, empty, or contain placeholder text. Concrete repeated misses: PR #18 used a `## Summary` / `## Context` structure; PR #19 also used a non-template structure. **The template is not optional.** The operator reads the PR description to decide approval — it must answer without follow-up: what changed, why, proof it works (real output), proof it's backward compatible, checklist. Diagrams (Mermaid, ASCII, screenshots) are preferred over paragraphs.
