...
0. **Never ask the user to perform manual web-UI steps that existing skills can automate.** When the path forward involves clicking through a web admin UI (Slack apps, Square developer dashboard, Google Cloud Console, GitHub settings, ADP, etc.), the FIRST thing to ask is "do we already have the building blocks to drive this with `skills/browser/` + `skills/credentials/` + a thin new orchestrator skill?" If yes, build the skill instead of writing a manual procedure for the user. Treat manual web-UI procedures as missing skills, not user homework. The collaborative-browser pattern (`skills/browser/collaborative.py`) handles auth/MFA/captcha mid-flow so partial automation is still a win.
   ...
   **The canonical pattern: `skills/<service>_app_provisioning/`** — when any third-party service has a self-serve developer dashboard for tokens/credentials/app-registration, build a provisioning skill named `skills/<service>_app_provisioning/` mirroring `skills/slack_app_provisioning/` (the reference implementation). Every such skill exposes `provision.py` (Playwright playbook generator) + `register.py` (post-Playwright finalizer that stores the captured secrets in Keychain via `skills/credentials/`, writes any agent/store-profile JSON, updates `config.yaml`).
   ...
   - 2026-06-02 — `gh` CLI keyring token went invalid. I told the user to run `gh auth login` (a manual web step) and then *asked* how to design durable auth, instead of driving the GitHub PAT flow with Playwright myself. User pushed back: "you can use playwright to do that… so I don't have to remind you of this skill." **Trigger to internalize:** any time a CLI/API call fails on auth (`gh`, gcloud, a portal) OR the next step is a web-UI click, STOP — reach for `user-playwright` MCP + `skills/credentials/` first; never hand the user manual web homework and never park it as a question. **Durable fix shape:** create a no-expiry classic PAT via Playwright, store it in macOS Keychain (HL#10), and `export GH_TOKEN` from Keychain in the shell rc so `gh` reads a static token and never re-prompts. ✅
   - 2026-06-03 — while scoping the BI dashboard (PR #16) I wrote "Looker Studio is a UI connect step I can't fully script" and offered the user click-by-click instructions. User pushed back: "so inaccurate… you have playwright skills which we have used so much. if a component doesn't offer an api/mcp way, we should use Playwright as last resort. refer to the principles where I mentioned more ownership. surprised I have to prompt." **Trigger to internalize:** NEVER tell the user a web tool "can't be scripted / is a manual step." The ownership ladder is non-negotiable and applies to BI tools, dashboards, any web product: (1) native API/SDK → (2) MCP → (3) IaC/declarative (e.g. Grafana dashboard JSON, Terraform) → (4) **Playwright via `user-playwright` + `skills/credentials/` as the LAST resort, but ALWAYS available.** "No API" never means "the user does it by hand" — it means "drive the browser." If a chosen tool's only creation path is manual clicking, either build the Playwright automation or weight that against tools with a real API when choosing. Bias tool selection toward agent-creatable (API/IaC-first) products precisely so the agent owns the whole lifecycle. ✅
...
18. **Opening or pushing to a PR OBLIGATES you to babysit it to merge-ready — automatically, without being asked.** The moment you run `gh pr create` or push to a PR branch, you own getting it green and mergeable: immediately read the `babysit` skill and enter its loop (poll CI → triage Claude/Bugbot/human comments → reply on every inline thread → fix scoped failures → re-push → re-watch) and do NOT end your turn until the PR is mergeable + all checks green + every comment triaged, or you hit a genuine blocker that needs the user. Concrete miss this happened on (2026-06-03, PR #14): the agent opened the PR and stopped, waiting for the user to say "babysit" — the user had to remind it twice. Documenting the loop in CONTRIBUTING is NOT enough; this always-applied rule is the enforcement. Treat "I opened a PR" and "my turn is over" as mutually exclusive. Also: a PR that touches code MUST have its own cost ledger committed (`pr_cost_ledger.py sync --pr <n>` then commit `metrics/pr_cost/`) or the **PR cost gate** check fails — record it as part of babysitting, not as an afterthought.
19. **Every new requirement/PR MUST start with a new Cursor chat. Your very first action is to run `start_pr_session.py` and surface the launcher.** When the user says anything like "let's work on X", "new requirement", "start PR N", "next task", or describes a new change — BEFORE writing a single line of code or doing any exploration, run:
    ```bash
    python3 scripts/start_pr_session.py --branch <branch-you-will-use> --requirement "<requirement text>"
    ```
    **NEVER assume or guess a PR number when starting a session.** GitHub assigns the PR number only when `gh pr create` actually runs, and **several chat spaces may be opening PRs at the same time** — so any number you pick up front (`--pr <next-n>`) will frequently be wrong, which breaks the cost gate (it keys off the *real* PR number) and collides with a parallel chat. The session is therefore tracked **provisionally by its branch** (`metrics/pr_cost/session-<slug>.json`, invisible to the report/gate). **Right after you open the PR**, bind the ledger to the real number — `gh pr create` prints it:
    ```bash
    python3 scripts/pr_cost_ledger.py bind-pr --branch <branch>   # auto-resolves the new PR # via gh
    python3 scripts/pr_cost_ledger.py sync --pr <the-new-number>  # capture build+review cost
    git add metrics/pr_cost/ && git commit -m "chore(cost): sync PR #<n> ledger"
    ```
    (`--pr <n>` on `start_pr_session.py` is still allowed for an *already-open* PR.) Then tell the user to open the printed **`session-<slug>-launch.html`** in a browser and click the button (or paste the **FIRST MESSAGE** block verbatim into a New Chat). **Never** tell them to type a placeholder like "Test PR N" — that hides the requirement and confuses the next agent. Do NOT continue building in the current chat after handing off. Rationale: reusing the current chat drags the entire conversation history as cache-read tokens into every new turn ($0.50/M on Opus). A fresh chat resets the counter, saving ~30% of build cost. **`start_pr_session.py` also stamps `session_started_at`** (carried across `bind-pr`) so `pr_cost_ledger.py sync` can attribute build cost to your branch/conversation window even when multiple chat spaces run in parallel (PR #16: $8.82 reported vs ~$0.68 actual without this). The platform limitation is real — you cannot open a new tab yourself; the launcher HTML + seed text is the lightest reliable handoff. **Isolated worktree per parallel chat:** when several requirements are in flight, run `python3 scripts/new_worktree.py --branch <branch> --requirement "…"` (or `git worktree add` manually) so each chat has its own checkout — never `git checkout` another branch in the shared tree while a sibling chat is building. Brief/launch sidecars (`*-brief.md`, `*-launch.html`, `session-*.json`) are gitignored handoff artifacts; only `PR-<n>.json` and `report.html` are committed. If ledgers show the same build session on two PRs, run `pr_cost_ledger.py dedup-sessions` before regenerating the report. Concrete misses to prevent: (a) continuing to implement a new requirement in the same chat as the previous PR's discussion, as on PR #15 (90% cache-read tokens, ~$3.76 avoidable); (b) opening the new chat with "Test PR N" instead of the seeded first message, as on PR #16; (c) **assuming PR #23 when the PR actually opened as #24, so the cost gate failed on a missing `PR-24.json`** (PR #24 — fixed by branch-keyed provisional sessions + `bind-pr`); (d) **branch_window attribution across overlapping chats double-counted ~$16 in the team report** (2026-06-04 — fixed by conversation-scoped sync + `dedup-sessions` + merged-only `report.html`).

20. **PR descriptions MUST use the 6-section template verbatim — CI fails otherwise.** Never pass `--body` with a free-form string to `gh pr create`. Always use `gh pr create --base main --fill` and fill in the opened template. The `PR Description` CI check (`scripts/check_pr_description.py`) will fail on PRs where any of the 6 sections are missing, empty, or contain placeholder text:
    - §1 What is the change (2–5 sentences)
    - §2 Motivation (why, linked to ticket/chat/PROGRESS.md)
    - §3 Design / Approach (how it's built — **diagrams strongly preferred** over paragraphs)
    - §4 End-to-end test (real commands + real output — "it should work" is not evidence)
    - §5 Backward compatibility — and proof (explicit yes/no + evidence; covers regression too)
    - §6 Checklist (each item checked [x] or explained)

    Concrete repeated misses: PR #18 used `## Summary`/`## Context`; PR #19 also used a non-template structure. **The template is not optional.** The operator reads the description to decide approval — it must answer without follow-up questions.

22. **Every GitHub API call (REST, GraphQL, `gh` CLI) MUST use the bot token — NEVER `ADI_TOKEN` or the personal `aditya2kx` PAT.** Concretely: `python3 - "$ADI_TOKEN"` in any shell one-liner is a violation. Always use `BOT_TOKEN=$(security find-generic-password -s github-bot-pat -a jarvis-agent-bot328 -w)`. The only exceptions are owner-only admin operations (enabling `allow_auto_merge`, setting branch protection, provisioning repo secrets) — use `ADI_TOKEN` for those only. Creating PRs, updating PR bodies, posting comments, pushing branches — all bot operations. Concrete miss (2026-06-03, PR #21 first attempt): I used `ADI_TOKEN` to call `gh api .../pulls` → PR was authored by `aditya2kx` → operator could not approve their own PR. Closed and recreated with bot token.

21. **The agent NEVER merges a PR. Only the operator merges.** Your job ends at *merge-ready*: all CI checks green, cost gate satisfied, every comment triaged. Do NOT run `gh pr merge`, `git merge`, or any equivalent. The operator does the final review and squash-merge to `main` — that is their explicit sign-off and cannot be delegated. Concrete miss (2026-06-03, PR #18): I ran `gh pr merge 18 --squash --auto` after CI went green. User correctly pointed to CONTRIBUTING § "The agent NEVER merges." **Trigger to internalize:** the moment all CI checks pass, your action is to report "PR #N is merge-ready — all checks green" and STOP. Never proceed to the merge step regardless of confidence level.
...
- **Keep `Playground/REQUIREMENTS.md` in sync.** This is the live progress tracker. (1) When the user verbally mentions a new requirement or bug in conversation, add it immediately with `python3 scripts/requirements_tracker.py add --req "short title"`. (2) When starting a PR for a requirement, run `start_pr_session.py --pr N --requirement-id <ID>` to auto-mark it 🔄 In Progress. (3) When a PR merges, `pr-cost-finalize.yml` auto-marks the linked requirement ✅ Done if the PR has a `req:<ID>` label or title tag — add the label when creating the PR. Never let the tracker go stale; it is the single source of what's done vs. pending vs. P0.

- **Prod/runtime runs on hosted infra — never on the laptop.** Any workflow that runs on a schedule or serves users must run entirely on hosted infra (Cloud Run, Grafana Cloud, BigQuery, GitHub Actions, etc.). The local laptop may only be in the loop for build, implementation, and one-time provisioning tasks (e.g. Playwright signup for a new service, minting a Keychain secret). If you find yourself adding a step that "works by running a script on the laptop," that is a gap — build the hosted equivalent (Cloud Run job, GH Action, scheduled Cloud Function). Concrete example: BQ backfill/model-materialize must run from the Cloud Run cron, not a laptop cron job. (2026-06-03, PR #16)
