## 2026-09-25 — Dolce's $18 raise and her admin half-hours (Issue #343, PR #345)

**Scope:** Dolce moved to $18/h on Mon 2026-09-21 as assistant manager, and she logs admin work as a separate ADP punch with a note. BHAGA had one current rate per employee (a raise would have repriced every closed period, including the Sep 7 draft already paid), the pay_info scrape could not match her ADP directory name to the alias, and ADP Timecard Notes were dropped at parse time, so admin punches drew tips like floor time.

**Key changes:** migrations 074–075 add `adp_wage_rate_history` (seeded 2000-01-01 from today's rates, so nothing reprices until a real change lands) and `vw_wage_rate_effective`; `vw_model_payroll_period` and the live labor views price each shift at the rate in effect on its date. History rows come from pay_info (dated by ADP "Added on", else the open period start), from the Earnings check's `period_start`, or from `python3 -m skills.adp_run_automation.wage_rate_history set`. pay_info matches aliases. `adp_punches.note` carries Timecard Notes; a punch whose note contains any `tip_exempt_punch_note_keywords` keyword (default `admin`, case-insensitive substring, by anyone) is a tip-exempt window unioned with training windows — paid in full, pool conserved. Nightly materialize now rewrites the whole open pay period so a mid-period note or rate lands on earlier days.

**Process note:** the first sandbox-live run on 2026-09-22 passed as a no-op. PR #344 had already set every `sandbox_runs` marker for that date, and its migration had taken number 073, so this branch's original 073 was skipped as "applied" and 074 failed on the missing view — while the check stayed green. Migrations were renumbered 074/075 after merging main. Both failure modes (marker reuse across PRs, migration-number collisions between open branches) are follow-ups.

**Evidence:** prod read-only parity of the rewritten views against current views: payroll 202/202 rows, daily 222/222, weekly identical to 6 dp. Simulating Dolce's real change: Aug 24 / Sep 7 periods unchanged at $16.25, the Sep 21 period $332.31 → $368.10; live daily labor unchanged through 09-19 and +hours × $1.75 on 09-22 ($12.13), 09-23 ($12.08), 09-24 ($11.58), 09-25 ($0.88). Real Timecard export 2026-09-25: 09-22 15:00–15:30 and 09-25 10:00–10:30 notes contain `Admin work`; a close-out note does not. 09-22 admin reproduction: her tip hours 8.5 → 8.0 with the day's pool conserved to the cent.
...
**Scope:** The operator asked to double-check three employees with zero solo hours, saying "am pretty sure Tina had solo hours". She was right for a reason the model could not see: on 2026-09-07 the manager was punched in from home, and occupancy counted every punched employee, so Tina looked accompanied for a shift she worked alone. She was credited **0.00h solo against an actual 4.62h** and paid base for all of it. Lindsay's punches are the only remote ones in the cycle; had every one of them been remote the cycle premium would have moved $16.15 → $43.52, so the exposure was ~3× the premium, not a rounding error. The miss was caught by operator memory, which is the part worth fixing.

**Key changes:** `SoloConfig.remote_days` carries `(date, employee)` annotations and `attribute_day` computes occupancy over present staff only, so a remote colleague neither masks someone else's solo block nor earns one. Remote hours are still **paid in full** — they land in `team_minutes` with `remote_minutes` (migration 072) reporting the slice, deliberately a subset rather than a fourth bucket so `solo + team == total` still holds exactly (invariant 11). Annotations live in `solo_shift_remote_days` in `store_config`, so marking a shift is a config edit and not a deploy (user-preferences #29); malformed JSON degrades to "everyone on the floor" with a breadcrumb rather than taking the nightly materialize down. `/payroll` and `/labor` gain an "of which remote" column, shown only when the window has remote hours, and the labor panel keeps a remote-only row so it cannot claim someone was alone while a colleague was clocked in.

**Decision:** an unannotated shift counts as **on the floor**. That is the pre-existing behaviour and it under-pays silently, which is the worse direction — but defaulting to remote would over-pay on every un-annotated shift in history. The honest fix is a real per-shift location source (#332, deferred by the operator), not a different guess; this change buys correctness for annotated shifts and makes the gap visible in the console meanwhile. Granularity is whole-day per employee: a half-remote day is not expressible yet, and no such day exists in the data.

**Process note:** the jam decision recorded in `solo_shift.py`'s own docstring — "Occupancy counts EVERY punched employee, including the manager" — was right about managers and silently wrong about location, and the emphasis on it made the second reading feel settled. A rule stated confidently in a docstring is not evidence that its edge cases were considered.

**Evidence:** 38 `test_solo_shift.py` green, 9 new, including a reproduction of the live 09-07 punch set asserting Tina at 277 min solo / $4.62 and Dolce gaining solo hours at $0 (already above the eligible rate); the randomized `solo + team == total` invariant now runs with ~30% of employees marked remote and also asserts `remote ≤ team`. 26 migration-071/072 structural tests, 102 materializer + labor tests, 26 console tests green. Applied to prod: cycle premium $16.15 → **$20.77**, Sep 7 total hours unchanged at 24.22h.

## 2026-09-21 — Cookie reuse was 0-for-7 against ADP because the profile was new every run (Issue #309)

**Scope:** ADP challenged for an SMS code on **7 of 7** runs in one morning, and all 7 had already logged `restoring trusted-device session`. Cookie persistence was not degraded, it was doing nothing: `launch_persistent` has been ephemeral since 2026-05-17 (the name is kept for caller compatibility), so a restored `SMSESSION` was arriving in a brand-new Chromium profile every time — indistinguishable from a stolen-cookie replay, which is the exact case a risk engine's step-up exists to catch. The operator was interrupted for six avoidable codes before anyone measured the ratio.

**Key changes:** `_portal_profile_dir()` gives each portal a stable profile (`~/.bhaga/browser-profiles/<portal>`) and `launch_persistent` uses `launch_persistent_context` when one is available, so the *device* persists rather than only its cookies; the saved jar now seeds only cookies the profile lacks, since a profile that has been through a login is authoritative. `BHAGA_SESSION_PERSIST` **defaults to on** — it was opt-in and set only in the deploy workflow, so every hand-run command silently started cold, and a flag whose only failure mode is an SMS to a human must not need remembering. Cloud Run stays ephemeral on purpose (`K_SERVICE`): its filesystem does not outlive an execution, so there the jar remains the only lever. Profile setup never fails a run — an unwritable root degrades to ephemeral with a breadcrumb.

**Process note:** `bhaga.mdc` had already named this fix ("persisting the whole browser profile") on 2026-09-15 as the lever that would remove the operator from the loop. It sat written down for six days while runs kept paying SMS codes, so the cost of the diagnosis was paid and then discarded. A recorded root cause that no one acts on reads the same as an unknown one.

**Evidence:** 28 `skills/_browser_runtime/test_runtime.py` tests green, 11 new — stable dir reused across calls, portals isolated, `K_SERVICE` forces ephemeral, opt-out honoured, unwritable root degrades, launch retries still apply on the persistent path, cookies seeded only when absent, corrupt jar never raises. 673 `skills/` tests green.
...
**Process note worth more than the fix:** this work was written on 2026-09-16, the issue was closed as "completed", and the diff then sat **uncommitted in a worktree** for two days while #317 — the PR it would have unblocked — stayed red. #317 is itself the fix for garage email noise, which recurred on 09-18 and was reported as a Tesla regression. Two separate incidents in one week traced to finished-but-unlanded work, so "closed as completed" now has to mean merged, not written. #317 eventually merged on 09-21 by *raising* its evidence instead (a zero-traffic tagged revision proving the credential rename, taking it to an unwaived `APPROVE`), which is the better outcome and leaves this change preventative: the contradiction is still latent for the next waived PR, and the two days #317 spent red were spent arguing with a gate rather than with the code.
...
**Key changes:** `send_garage_email` requires `K_SERVICE` (Cloud Run) or `GARAGE_NOTIFY_FORCE=1`, so only the deployed service can mail; `conftest.py` strips both credential sets for every garage test. Garage credentials are now `GARAGE_GMAIL_*` with no fallback to the bare `GMAIL_*` that `cloud/pup_watch` mails from (same Secret Manager secrets, service-scoped env names) — the two notifiers are independent, so neither service's shell can drive the other's mailer. Gmail only on a real live `opened`. `/simulate/enter` with last Tesla metres already inside the fence is `skip_already_inside`. Firestore persists `last_open_ts` so cooldown survives Cloud Run restart. `open_error` arms cooldown; already-open / simulate / errors stay in Cloud Logging.
...
**Evidence:** 80 garage unit tests; `should_email` matrix; the reproduction (`GMAIL_*=dummy pytest cloud/tesla_aladdin_garage/`) goes 3 live send attempts → 0, and stays 0 even with `K_SERVICE` forced into the suite. No live `simulate/enter` (opens Big Peach). `verify.py`'s secret scan now ignores `KEY=secret-name:latest` Secret Manager references, which otherwise flag every Cloud Run env edit on the password pattern.

**2026-09-18 — it happened again, and that is the argument for a root-level guard.** The operator reported "getting those tesla emails again" and read it as a Tesla regression; nothing about Tesla had changed. The bursts (six per burst, several subjects saying `simulated`) matched, to the minute, `pytest cloud/` runs in a shell that had the production `GMAIL_*` exported for pup-watch's live evidence — i.e. **exactly the bug this PR fixes, still unfixed because this PR is unmerged.** A fix that is merged-ready but unmerged protects nobody; that is now twice in two days (see the `pup-watch` #322 entry below). Two things were added here rather than filed away: (1) a **root `conftest.py`** that strips every outbound credential for every test in the repo, since per-module discipline is the thing that failed — `test_notify.py` did clear the credentials, but only for its own tests, while `test_app.py`'s `/simulate/enter` (the actual sender) never touched them, and pup-watch has the same shape with no runtime gate (#320). It also strips `K_SERVICE` and `GARAGE_NOTIFY_FORCE`, so a shell that sets them cannot re-open the hole this PR's runtime gate closes. (2) The **read** side of the shared mailbox: scoped credentials stop pup-watch mailing *as* the garage, but pup-watch also *polls* that mailbox for `start`/`stop` replies, and garage mail arrives from the allowlisted address — so it reached the command parser, one unlucky subject line from switching the dog camera off. Garage mail now carries `X-Jarvis-Garage` and is skipped before parsing (labelled, so a burst cannot crowd a real command out of the 10-message query window).

**The credential rename now has live proof, without opening the door.** Claude's blocking gap across three review rounds was the one change that fails silently — if the rollout does not remount the secrets under `GARAGE_GMAIL_*`, the sender logs `notify_unconfigured` forever and no real open ever mails — and the only true e2e (`POST /simulate/enter`) physically opens Big Peach. Resolved with a **zero-traffic tagged revision** built from the PR's own commit (`pr317-d47d571`), deployed with `GARAGE_WORKER=0` / `ALADDIN_DRY_RUN=1` so it cannot reach the door: `/health` on the tagged URL returned `notify: {configured: true, credentials: "GARAGE_GMAIL_*", missing: [], unscoped_present: false, runtime_allowed: true}`, while the serving revision (`00041-ttm`, still 100% of traffic) continues to mount the bare `GMAIL_*` — the before/after of the rename, side by side, on real revisions. Zero mail delivered during the probe. Recipe recorded in RUNBOOK so the next env change to this service is provable the same way.

**Evidence for the recurrence fix:** 479 `cloud/` tests green, then the decisive run — the full suite with **real** `GMAIL_*` *and* `GARAGE_GMAIL_*` *and* `GARAGE_NOTIFY_TO` *and* `K_SERVICE` all exported (worst case, gate defeated): inbox message count over the preceding hour was **30 before and 30 after, 0 delivered**. Before the guard the same run delivered six.
...
**Email control, folded in on operator request.** curl-with-a-token is useless at daycare drop-off, so replying `start` / `start 4h` / `stop` / `status` to any pup-watch email now controls monitoring (`control.py`, `sessions.py`). Polling the mailbox we already own beat standing up an inbound-mail path — no MX, no webhook, no new secret; the same Gmail OAuth trio that sends the alert reads the answer (it already had `gmail.modify`). Idempotency is Gmail's unread flag rather than state of ours that could drift. The HTTP handlers and the email path now share `sessions.start/stop` so "start" cannot come to mean two different things. Cost: the mailbox must be read *before* the session check — otherwise an emailed `start` could never wake an idle service — so an idle tick went from ~1ms to ~1.05s, taking the worker from ~0% to ~16% of the Cloud Run free tier. Accepted deliberately; the documented lever is polling every Nth idle tick.

**Two security models tried; the second is the right one.** The first gate required `spf=pass` + `dkim=pass`, which live testing proved refuses the *operator's own commands*: Gmail writes no `Authentication-Results` on mail an account sends to itself. The `SENT` label is both the fix and the stronger signal — only the account holder can produce a sent message, whereas SPF/DKIM merely prove a `From` domain. Third-party senders still need SPF+DKIM. A related trap caught in review of my own code: keying "ignore our own mail" on the `SENT` label would have silently dropped every command, since the operator sends *from* the notification mailbox — outgoing mail is now stamped `X-PupWatch` instead. Commands must also be the first unquoted line, so "stopped raining, he loved it" is not a command, and anything older than 30 min is discarded rather than applied late.
...
**Post-deploy, same day — three bugs real usage found that no evidence run did.** (1) The deploy workflow **failed on its first execution**: `--set-env-vars` splits on commas, but `PUPWATCH_REFERENCE_URIS` and `PUPWATCH_NOTIFY_TO` are themselves comma-separated, so gcloud read a photo path as an env var name; and `scheduler jobs create` takes `--headers`, not the `--update-headers` the workflow hardcoded. Prod was brought up by hand from the already-built image; #322 makes it reproducible with tests that fail on the pre-fix file. (2) **Email control never worked for the operator.** Discovery queried `is:unread`, but **Gmail marks a message you compose yourself as already read** — his replies arrived `['IMPORTANT','SENT','INBOX']` with no `UNREAD`, so every command was ignored with no ack and not even a rejection log. The pre-merge evidence passed only because API-sent mail *does* arrive unread. Idempotency now uses a hidden `pupwatch-handled` label, which has no hidden dependency on who sent the message; our own alerts are labelled but never marked read, since the unread badge is the notification. (3) `start` now means **until an explicit `stop`** (operator request) — `session_max_hours` bounds only fixed-length sessions, open-ended ones get `session_absolute_max_hours` (7 days) and hitting it emails rather than going silent. A *malformed* duration falls back to the bounded ceiling rather than becoming unbounded. Cost consequence recorded: continuous monitoring is ~9,000 vCPU-s/day and leaves the free tier in under three weeks, so `stop` matters.

**2026-09-18 — the open-ended session died in silence, which is the worse half of the bug.** The operator reported no alerts while his pup had been out. Monitoring had stopped itself at **Thu 6:00 AM**, `stopped_by=session_expired_max_hours`: #322 (which exempts open-ended sessions from `session_max_hours`) was still unmerged, so prod ran the #318 image and applied the 12h ceiling to a session explicitly started as "until I say stop". Every tick since returned `no_active_session` in ~1ms, which is why the logs looked empty rather than broken. Restored without a deploy by writing the `session_max_hours` overlay (168h) to the Firestore config doc and starting a fresh session — the tunable was already runtime-changeable (user-preference 29), so no code was needed to recover. **The lesson is not the ceiling but the silence:** a stopped watcher is indistinguishable from a working one that has seen nothing, so `worker.tick` now announces *every* automatic stop, not just the absolute-max case. Second-order lesson for the dev loop: a fix that is merged-ready but unmerged protects nobody — #322 sat green for two days while the bug it fixes was live in prod.

**2026-09-18 (later) — second yard added, and the veto stack inverted.** The operator asked to also watch the Montrose outside-big yard (`5ee27f3358677`) and for the email to name which link caught him. The link turned out to be the **group-play** yard — a dozen dogs and a handler in the first frame pulled — so under the existing "more than one dog ⇒ not him" rule it would have polled all day and alerted never: the same silent-uselessness shape as the morning's bug, so it was raised rather than shipped. His rule resolved it and simplified the design: *"if there's only 1 dog in any of those yards, I want to be notified. you can additionally say if it looks like chai or not"*, plus *"there could be humans with him, that's fine"*. So **one dog in view is now the only veto**; people were already ignored; and both remaining gates became advisory — the cream-coat threshold (fails in poor light) and the Gemini re-ID (cannot separate him from similar cream dogs at crop resolution, measured 2026-09-16). Both were previously able to cancel an email, i.e. two measured-unreliable checks held veto power over the one thing the system exists to do. Subject now hedges honestly ("A dog is out alone in the B YARD — may not be him (41%)") and the body carries the yard link. A speculative `expect_alone` per-camera flag was added and then removed once his rule made it unnecessary.

**2026-09-18 (post-merge) — the second camera did not fit, and the cost model was ~10x wrong.** #322 merged and deployed green (the repaired workflow's first real run), `/health` showed both yards. But prod then logged `The request was aborted because there was no available instance` and **`b-yard` poll lines were missing**: on `--cpu 1` a two-camera tick took 100-120s against the 60s schedule, so ticks overlapped and were aborted. Worse, the deploy asked for 1 vCPU while setting `PUPWATCH_ORT_THREADS=2`, oversubscribing the detector against itself. Raised to 4 vCPU (`--cpu` and threads now pinned equal, with tests that fail on the old file): the same tick measures ~52s and both yards poll every tick. Investigating that exposed a much bigger error: the README's cost model was timed on a laptop and ignored that Cloud Run bills `vCPU × wall time`. Real numbers: an active tick is ~208 vCPU-seconds, so **one full day of monitoring exceeds the entire monthly free allowance** (~$1.30/day beyond), and idle ticks alone now consume ~63% of it. "$0/month" was wrong; docs corrected. Follow-up: a smaller detector (yolov9-c at 130MB is most of the 52s) would cut both the latency and the bill.
...
**Decision:** premium hours are keyed as a second ADP rate line, and the draft can now do it itself behind `BHAGA_ADP_SOLO_RATE2` (off by default) after the operator asked for the automation and the console numbers to agree. Invariant 6 is about never Approving/Submitting, not about never typing: the draft already fills money columns. Hours are different — this is the only path that rewrites them — hence the flag, a gate on the hours guardrail, and per-employee verification by re-reading the grid. FLSA weighted-average overtime on mixed-rate weeks is deferred to #315; no eligible employee hit overtime this cycle.

**Two things the first live proof caught,** run against a real draft that applied nothing and left nothing behind. The row overflow trigger is an `sdf-action-menu` element rather than a `button`, so the selector matched nothing and all six employees failed safe. Worse, the split ran with 7 hours-guardrail failures outstanding, because it gated on `fill_ok` — which deliberately stays true through a guardrail failure so money columns can still fill. Menu items are now clicked by ADP's `data-test-id`, and the split has its own gate, pinned by a test.

**The staleness gap, and why it mattered most.** Solo hours are materialized separately from the punches they derive from, so they drift when that step is skipped — and a stale premium is *understated*, not absent, which is indistinguishable from a quiet fortnight. Prod's nightly does not build solo hours until this deploys, so the table sat two days behind current punches and the closing cycle read 12.24 h/$12.24 against an actual 20.16 h, with Jacob Garcia ~$8 short. Both surfaces now compare the period's punch dates against `model_solo_hours_daily`: `/payroll` marks the card stale and names the missing dates, and the draft refuses to key rate-2 even with the flag on. Verified live — the warning named 09-19 and 09-20, then cleared after a re-materialize.

**Wages are shown split by rate line, not as an uplift,** on operator request. `/payroll` headlines now read Wages (blended, with the effective $/h) → Primary wages → Solo wages, and the per-employee table matches. Solo wages are the hours at $16.25 — the whole rate-2 line, which is what ADP shows — rather than the $1.00 difference; the standalone "Solo premium" card is gone because folding the premium into Wages is what keeps the headline cards summing to Total pay. Primary carries overtime so that primary + solo reconciles to total wages exactly (mixed-rate OT is #315); primary is computed as the remainder rather than a second product, so rounding cannot open a cent-wide gap between the parts and the whole.

**Evidence:** final cycle figure 18.47 eligible solo hours / $18.47 premium, reconciled per-date against prod; the split renders live at $5,813.63 primary + $300.13 solo = $6,113.76 wages at $17.53/h blended, and wages + tips + bonus + perks ties to the $7,094.32 Total pay card; console, `vw_solo_hours_period` and the draft packet agree, and Hillary Huynh's 27.80 h matches the ADP grid exactly. The earlier 12.23 h figure corrects the 14.75 h jam estimate (which predated the manager-occupancy rule and the minimum-block threshold) but was itself stale — the staleness guard exists so that cannot recur silently. Unit coverage per named scenario, migration regression pin for the open-period `pay_period_end` truncation, `full-live` armed on 2026-09-08, operator sign-off at localhost. Live proof #2 of the split actually applying is still outstanding.
...
**Key changes:** Expand `cloud/webhook/Dockerfile`; mount `CLICKUP_PAT` + `BHAGA_DATASTORE` on webhook deploy; scheduler `bhaga-team-pulse` already created; rotated `sandbox-trigger-token` (v3; prior disabled) after describe leak + jam re-exposure; scheduler header re-wired (Content-Type=application/json). Pre-merge evidence: Cloud Build import `ok team-pulse`; live dry-run on rev `bhaga-webhook-00171-8kn` (`i223-pr224`) → `status=dry_run`, 5 leaderboard rows.
