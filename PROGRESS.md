...
**Iron Man rebuild (operator review: wanted RDJ-era / JARVIS / Tony Stark, "modern polished as in movie"):** replaced the flat red-and-brass livery with a Mark-43 one — traced faceplate on the hood, lit eye slits across the front fascia, arc reactor plus JARVIS reticle on each front door, repulsors at the quarters and tail; palette moved to champagne gold over deep candy red. Two findings worth keeping: (1) the hood island **forks into two horns below y=310** for the windscreen scuttle, so emblems sized to its bounding box get sliced open — `test_faceplate_clears_the_hood_scuttle_notch` pins it, and every reactor radius is now bounded by the panel's *inscribed* circle since the bloom reaches 1.30x and the reticle brackets 1.12x; (2) `paint.ticks()` must measure perpendicular distance to each tick's ray, not angular width — thresholding on angle makes ticks sub-pixel at large radii and they render dashed.

**Helmet + JARVIS retrace (operator review: "iron man mask and jarvis doesn't look official"):** the mask was hand-approximated, so it was rebuilt the way the bat had been — segment a flat front-elevation reference into its shell / plate / emitter regions, contour each, RDP-simplify, keep the right half, mirror. The finding: a helmet is unrecognisable from its **silhouette** (that path yields a gold egg) and unmistakable from its **plate break-up** — brow plate with a crown V notch, cheek plate wrapping under both eyes, chin plate, crescent jaw plates, and the shell red left showing between them *is* the sockets and mouth, so neither is drawn. Shading likewise comes from geometry (each plate domed off its own outline, bevelled bright on the crown-facing edge) rather than hand-placed highlights. `FACE_ASPECT` 1.22 → 1.4832, hood width 154 → 130. The nose fascia now reuses the helmet's own eye via `emblems.slit_outline()`. JARVIS moved from concentric circles (reads as a clock face) to the film grammar: segmented rings via a new `paint.dashes()`, four square corner brackets, two gauge arcs, tick ladder, composited *over* the reactor. Two mechanical findings: `paint.ring()` feathers both edges, so any stroke under ~2.4px silently fades instead of sharpening; and the reactor's core triangle needs a per-panel `rotate`/`flip` because the atlas points "up the car" a different way in every zone — stamped flat, door and tail reactors came out apex-down. Regression tests pin plate/eye containment, eyes-land-in-shell-not-on-gold, and ring segmentation.
