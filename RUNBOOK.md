...
Leaderboard source for the **scheduled** morning job is the **open**
`model_review_bonus_period` rollup (written by nightly `process_reviews`).
Operator Console `/automations/team-pulse` Preview / Post once use a Payroll-style
`?period=` filter (same period list as `/payroll`) so you can compose against a
closed biweek and DM yourself; the morning schedule is unchanged. The console shows
...
- [ ] `jarvis-agent-bot328` PAT is in Keychain (`security find-generic-password -s github-bot-pat -a jarvis-agent-bot328 -w`). This bot is a Write collaborator on the repo and is used for all agent pushes and PRs.
      The bot account has 2FA (TOTP) enrolled (enrolled 2026-06-28); TOTP secret is in Keychain `github-bot-totp`; recovery codes in `github-bot-recovery`. The `origin` remote is tokenless (`https://github.com/aditya2kx/jarvis.git`) and `gh auth setup-git` provides credentials via the Keychain-backed `GH_TOKEN`. When rotating the bot PAT, write the **same** new classic token (`repo`, `workflow`, `read:org`) to Keychain `github-bot-pat` **and** Keychain `github_pat` (account `jarvis-agent-bot328`) **and** repo secret `ADMIN_PAT` (`printf '%s' '<new>' | gh secret set ADMIN_PAT --repo aditya2kx/jarvis`); prove laptop + an `ADMIN_PAT` Actions job as the bot; only then revoke old bot PATs. Full procedure: `docs/contributing/push-gotchas.md` §2.
...
| Proving a deploy **before** it serves traffic | A credential rename or env change here fails *silently* (the sender logs `notify_unconfigured` and no real open ever mails), and the only true e2e — `POST /simulate/enter` — physically opens Big Peach. Verify on a zero-traffic tagged revision instead, with the worker loop off so it cannot reach the door: `gcloud run deploy tesla-aladdin-garage --image <branch image> --no-traffic --tag <name> --set-env-vars "GARAGE_WORKER=0,ALADDIN_DRY_RUN=1,GARAGE_PERSIST=0,…" --set-secrets "GARAGE_GMAIL_*=…"`, then `curl -sS "https://<name>---<service-host>/health" -H "X-Garage-Token: $GARAGE_ADMIN_TOKEN"` and read the `notify` block. 100% of traffic stays on the serving revision throughout (`--format='value(status.traffic)'` to confirm). Delete the revision when done. |
