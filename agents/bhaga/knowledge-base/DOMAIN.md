...
|---|---|---|---|
| `vw_order_reco_next_dates` | one row per future registered date, max 2 | `delivery_date`, `slot` (1 or 2) | The next two FUTURE (`>= CURRENT_DATE('America/Chicago')`) distinct dates from `inventory_restock_schedule`, oldest first. Past dates silently dropped. Zero rows if nothing future registered; one row if only slot 1 exists. |
| `inventory_order_reco` (physical TABLE) | one row per (store, Slot, Item) + 1 TOTAL row per slot | `store`, `Slot`, `Item`, `Current Qty`, `Avg per day`, `On Hand at Restock`, `Order Tubs`, `Order Weight lbs`, `After Restock`, `Days Left After Restock`, `_ord`, `refreshed_at`, `delivery_date` (041) | **Materialized recommendation (Option D)** — refreshed by `core/order_reco.py::refresh_order_reco()`, never written directly by Grafana/handler. A single query computing both slots (nested TVF call, flat combined TVF, window-function totals, `GROUP BY ROLLUP`) all failed in prod-live testing with `400 Resources exceeded ... query is too complex` once slot 2 re-derives slot 1's entire `GENERATE_ARRAY` water-fill chain in the same plan. Each slot is instead computed by a SEPARATE table function call/BQ job and the row lands here; slot 2 reads slot 1 back from this table (a cheap flat scan), not from a nested TVF call. Migration **041** binds each row to `delivery_date` and filters next dates with `> today CT` so Operator Console headers cannot label Slot quantities from a prior calendar date. |
