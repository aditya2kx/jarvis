...
- **`solo_minutes` / `solo_hours`** — minutes an employee was the **only** person punched in,
  counting only contiguous runs at or above `solo_shift_min_block_minutes`. Occupancy counts every
  punched employee including the salaried manager: being excluded from the tip pool or from labor %
  says nothing about whether a colleague is physically in the shop.
...
- The four tunables live in `bhaga.store_config`, not in code: `solo_shift_min_block_minutes`,
...
**Migration 031 additions** (`core/migrations/031_order_reco_dual.sql`, Issue #137 — dual-date reco + Grafana, PR B of 2, **Option D: materialized**). Supersedes migration 029's `tvf_order_reco`/`oa_ship_days` Grafana variable (both left in place, unused reference fallback — ship time is now the real calendar gap to a registered date, and capacity is `order_reco_max_tubs` in `store_config`, not a Grafana variable).

| BQ object | Grain | Key columns | Purpose |
|---|---|---|---|
| `vw_order_reco_next_dates` | one row per future registered date, max N | `delivery_date`, `slot` (1..N) | The next planning dates from `inventory_restock_schedule`, oldest first. Cap = `order_reco_max_slots` in `store_config` (**default 4**, migration 052 / Issue #215). **Closing-aware today (051):** includes `delivery_date > today CT`, plus **today** only while no `inventory_closing_daily` base row exists for today. Past dates silently dropped. |
| `inventory_order_reco` (physical TABLE) | one row per (store, Slot, Item) + 1 TOTAL row per slot | `store`, `Slot`, `Item`, `Current Qty`, `Avg per day`, `On Hand at Restock`, `Order Tubs`, `Order Weight lbs`, `After Restock`, `Days Left After Restock`, `_ord`, `refreshed_at`, `delivery_date` (041) | **Materialized recommendation (Option D)** — refreshed by `core/order_reco.py::refresh_order_reco()`, never written directly by Grafana/handler. Slot 1 via `tvf_order_reco_slot1`; slots ≥2 via `tvf_order_reco_slot_n(max_tubs, target_slot)` chaining from the prior materialized slot (052). **Write-then-swap (067 / Issue #261):** INSERT a shared `refreshed_at` generation, then DELETE rows with a different `refreshed_at` — no empty table between DELETE and INSERT. `s_prev` QUALIFYs the latest generation per item. Grafana `vw_order_reco_combined` still pivots the first two slots; Operator Console paints only dates whose item Order tubs sum to TOTAL. Migration **055**: Estimated dates honor per-item pins in `inventory_order_tub_overrides` (Manual Source badge); Actuals still win whole-date. |
| `inventory_order_tub_overrides` | one row per (store, delivery_date, item) | `quantity_tubs`, `updated_by`, `updated_at` | Issue #225 — Manual Order Tubs pins on Estimated dates only. Replace-per-date from Operator Console batch drawer; shrinks water-fill budget; cleared when schedule date is deleted/replaced. |
| `tvf_order_reco_slot1(max_tubs INT64)` | one row per (store, item) + 1 TOTAL row | same columns as `tvf_order_reco` (migration 029), `On Hand at Restock` replaces `On Hand in 10d`, `_ord` added | Water-fill for slot 1's date. `ship_days = DATE_DIFF(slot1_date, today, DAY)`. Shows the estimated allocation until `inventory_restock_orders` has rows for slot 1's date, then shows those actuals verbatim (0 for any item not in the upload). |
| `tvf_order_reco_slot2(max_tubs INT64)` | one row per (store, item) + 1 TOTAL row, or zero rows if no slot-2 date | same columns as slot1 | **Chained via the materialized table, not a nested TVF call**: reads slot 1's row from `inventory_order_reco WHERE Slot = 1` — on-hand-at-slot-2-arrival assumes slot 1's recommendation (estimated or actual, whichever it currently shows) already arrived and was consumed until slot 2's date (`DATE_DIFF(slot2_date, slot1_date, DAY)`). Independently applies the estimated-vs-actual override for slot 2's own date. |
| `vw_order_reco_slot1` / `vw_order_reco_slot2` | pass-through over `inventory_order_reco` | same as the TVFs, minus `store`/`Slot`/`_ord`/`refreshed_at` | Defined for `refresh_date` freshness tracking (`status.py`); no longer rendered by any panel since migration 032 (see below) combined both into one table. |

`order_reco_max_tubs` (default 120) lives in `store_config`, read by `refresh_order_reco()` — change via `/bhaga-cloud config set order_reco_max_tubs <N>` (recomputes immediately, see `cloud/webhook/handler.py::_handle_config_set`). Refresh triggers: nightly `daily_refresh.py` step `refresh_order_reco` (after `ingest_inventory`), the restock modal's `view_submission` handler (after any schedule/orders write), and `order_reco_max_tubs` config-set — all three converge on `core/order_reco.py::refresh_order_reco()` (handler.py duplicates the same 3 statements inline as `_refresh_order_reco` since it's a standalone deploy unit — see its docstring).

**Migration 032 addition** (`core/migrations/032_order_reco_combined.sql`, Issue #137 iteration — combined table). Per-operator feedback, replaces the two-panel design (81/82, `vw_order_reco_slot1`/`slot2`) with one panel (83) reading a single view. A pure pivot — no water-fill logic re-derived here, `inventory_order_reco`/the TVFs/`refresh_order_reco()` are untouched.
