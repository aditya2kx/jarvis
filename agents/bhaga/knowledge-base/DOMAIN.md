...
| `vw_order_reco_slot1` / `vw_order_reco_slot2` | pass-through over `inventory_order_reco` | same as the TVFs, minus `store`/`Slot`/`_ord`/`refreshed_at` | What Grafana panels 81/82 actually read — pure `SELECT * FROM ...` (see `scripts/check_grafana_no_logic.py` `MUST_BE_CLEAN = {79, 81, 82}`). |
