...
| BQ object | Grain | Key columns | Purpose |
|---|---|---|---|
| `vw_order_reco_combined` | one row per (store, Item) | `Item`, `Current Qty`, `Avg per day`, then per date N∈{1,2}: `On Hand N`, `Order Tubs N`, `Order Weight N`, `After Restock N`, `Days Left N`, `Source N` | Self-join pivot (`FULL OUTER JOIN` of `inventory_order_reco WHERE Slot=1` and `Slot=2` on `Item`) so shared identity columns (`Item`/`Current Qty`/`Avg per day` — identical across slots) appear once and each date gets its own column group. `Source N` = `'Actuals'` if `inventory_restock_orders` has any rows for that date, else `'Estimated'` (`Source 2` is `NULL` when no second date is registered). What Grafana panel 83 actually reads — pure `SELECT * FROM ...` (see `scripts/check_grafana_no_logic.py` `MUST_BE_CLEAN = {79, 83}`). Panel 83 freezes the first 3 columns (`options.frozenColumns.left = 3`) so `Item`/`Current Qty`/`Avg per day` stay visible while scrolling the date column-groups. |
