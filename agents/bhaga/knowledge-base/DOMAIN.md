...
| BQ table / view | Grain | Key columns | Purpose |
|---|---|---|---|
| `vw_inventory_order_assistant` | one row per (store, item) | `category`, `store`, `item`, `current_qty`, `reported` (CT timestamp string), `reported_date`, `last_restock_date`, `usage_7d_total`, `avg_daily_usage`, `days_left`, `days_considered`, `excluded_days` | Analytical view for the Order Assistant table. Per-base: current stock, usage/avg/days-left over the **last 7 eligible unique-weekday transitions** (one per Mon–Sun, most recent non-outlier eligible day per weekday; downward-only: `GREATEST(prev_close - curr_close, 0)`). Eligibility: previous reading exists, no submission gap (date = prev + 1), qty ≥ 1 tub, store open (orders > 0), not a restock day (curr − prev ≤ 1.0). **Outlier exclusion (robust-z):** after eligibility, a trailing 60-day window computes per-item median + MAD; days where `|robust-z| > 2.5` (guard: ≥5 eligible days AND MAD > 0) are excluded before the unique-weekday pick — mirrors `forecast.py compute_outlier_stats`. `days_considered` = `MM/DD Ddd (x.xx)` per eligible non-outlier day (consumed amount included). `excluded_days` = recent (30-day) excluded transitions with reason labels (restock X→Y, gap N days, closed, qty<1, outlier x.xx vs med y.yy). `days_left = current_qty / avg_daily_usage` (NULL if usage = 0). Grafana panel 79 drops the `category` column (display-only; always "base" for this filter) and adds a footer row summing `Current Qty`, `Usage 7d`, and `Avg per day`. Backs Grafana "8. Order Assistant" panel 79 (analytics table). |
