...
| `ingest_inventory.py` | **Order Assistant ingest.** Fetches ClickUp closing-form tasks from the "Closing" list, parses per-base quantities via `skills/inventory_parse/parse.py`, and MERGE-upserts into `bhaga.inventory_closing_daily` on natural key `(store, source_task_id, field_id)`. Incremental via `MAX(submitted_ts)` high-water mark; supports `--backfill-from YYYY-MM-DD` and `--date YYYY-MM-DD`. Non-fatal in the nightly — failure leaves today's inventory data stale but does NOT abort the tip/payroll run. Grafana sources: `vw_inventory_base_latest_daily` (freshness-checked in GRAFANA_VIEWS) and `vw_inventory_order_assistant` (panel 79 analytics table — per-base current stock, last-7-eligible-days usage, avg/day, days remaining, restock date; see migration 028). Panel 81 is the order-recommendation table — max-min water-fill over `vw_inventory_order_assistant`, entirely in `rawSql` driven by `oa_ship_days` / `oa_max_tubs` Grafana variables (no stored view). Panel 81 also shows `Order Weight (lbs)` per row (`order_tubs × {Açaí 18 \| other bases 20} lbs/tub`, Blade = NA) and a pallet-aware TOTAL (`+50 lbs` per 40-tub pallet, `CEIL`-rounded) — computed inline in `rawSql`, no new BQ view (`status.py` `GRAFANA_VIEWS` unchanged). |
