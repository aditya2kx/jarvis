...
4. **Load ADP → BigQuery (primary)** (`backfill_from_downloads.py --skip square`, requires
   ...
   `adp_open_shifts` (open / unassigned slots from the same JSON's `open_shift_cells`, one row per
   `(date, slot_index)`, purged per scraped `week_start` — Issue #342; `RUNBOOK.md` § ADP open shifts),
...
8b. **ADP payroll draft** (Issue #251, `BHAGA_ADP_PAYROLL_DRAFT=1` only): Monday 07:00 after a Sunday period-end, **headless** Start → fill NQCCTips/Bonus/Misc → Preview (leave draft). Never Approve. Writes `payroll_draft_runs` (Preview URL). Default env unset. Visible browser only with `BHAGA_ADP_HEADED=1`.
...
| Script | Role |
|---|---|
| `status.py` | **Run this first for any operational question about whether a run landed.** Read-only freshness checker across all three layers — Sheets (`data_window_end`, `daily`, `tip_alloc_daily`), BigQuery (model_* + raw tables), and Grafana BI contract views (vw_*). Prints a compact table and exits nonzero if any layer is missing the date (alert/CI usable). Anti-drift: its declarative registry is kept in sync with `core/migrations/*.sql` by CI-enforced coupling in `scripts/check_doc_freshness.py` and by sync tests in `test_status.py`. `GRAFANA_VIEWS` registry includes `vw_kds_order_quality_by_source_daily` (migration 025, per-source P95 KDS time backing panel 51) and `vw_labor_weekly` (migration 002, per-employee weekly hours backing panel 38 — Weekly Shift Hours per Person). Migration 033's `recognition_bonuses` is intentionally NOT in `BQ_TARGETS` either — it's an operator-console-only write table (Payroll & People screen), has no `vw_*` Grafana view, and is read via the console's own typed queries, not this status doctor. Migration 030's `inventory_restock_schedule`/`inventory_restock_orders` are intentionally NOT in `BQ_TARGETS` — like `store_config`, they're operator-input tables with no freshness cadence (rows only appear once `/bhaga-cloud restock` is used). Migration 034's `vw_kds_per_item_min` is likewise intentionally NOT in `GRAFANA_VIEWS` — it's a console-only view (raw per-ticket KDS per-item minutes at full float precision, used by `apps/operator-console/`'s `orderQualityByGrain` to recompute weekly/monthly Order Quality percentiles from source rows) that Grafana's `dashboard.json` never references. Migration 035/036's `vw_inventory_base_runway` (Issues #156/#164) is also intentionally NOT in `GRAFANA_VIEWS` — Operator Console `/inventory` Base runway table only (burn-down days left; dual Restock/Stockout/Status 1+2 from Actuals-only dates; Risky when restock empty or stockout < restock); freshness of underlying `vw_inventory_order_assistant` + restock orders is already covered. Migration 037's `plaid_items` / `plaid_transactions` / `vw_plaid_spend_by_category_daily` (Issue #158) are likewise intentionally NOT in `BQ_TARGETS`/`GRAFANA_VIEWS` — Operator Console Accounting (+ `skills/plaid_api` sync) only; same console-only class as `recognition_bonuses`. Migrations 043 (`plaid_accounts` masks) and 044 (`plaid_transactions.is_internal` + view exclude for checking↔card transfers, Issue #168 Phase A) stay in that console-only class — no new `BQ_TARGETS`/`GRAFANA_VIEWS` entries. Migrations 045/046 (Issue #160: `plaid_taxonomy_nodes` / `plaid_category_rules` + effective-category rewrite of `vw_plaid_spend_by_category_daily`) likewise stay console-only — no new registry targets. Migration 041 adds `delivery_date` on `inventory_order_reco` and date-joins the existing slot/combined views — no new `BQ_TARGETS`/`GRAFANA_VIEWS` entries (freshness still via `refresh_date` on the 031/032 views already registered). Migration 042 prefers tub `raw_text` over box/case packaging when collapsing duplicate display-name fields in `vw_inventory_base_latest_daily` / `vw_inventory_order_assistant` (ClickUp dual Mango) — same views, no new targets. Migration 048 (Issue #194) adds `inventory_usage_day_overrides` + `vw_inventory_usage_day_audit` and joins overrides into `vw_inventory_order_assistant` — console `/inventory` only; no new `BQ_TARGETS`/`GRAFANA_VIEWS` (OA freshness still via the existing OA view target). Migration 049 (Issue #206) rewrites `vw_model_payroll_period` to LEFT JOIN aggregated `recognition_bonuses` (recognition_bonus / recognition_reason; folds into `est_total_pay` + `bonus_diff`) — same view already in `GRAFANA_VIEWS`, no new registry target. Migration 059 (Issue #251) adds `employee_perks` (console-only, cents) and rewrites that same payroll view (PT tip-alloc UNION FT punches; rate/OT/perks) — still no new `GRAFANA_VIEWS` target. Migrations 074/075 (Issue #343) add `adp_wage_rate_history` / `vw_wage_rate_effective` (change-only, no freshness cadence — same class as `store_config`) and republish `vw_model_payroll_period` / `vw_labor_*_live`, already covered. CLI: `python3 -m agents.bhaga.scripts.status --store palmetto [--date YYYY-MM-DD] [--json] [--check-schema]`. |

| `daily_refresh.py` | **Nightly orchestrator.** Issue #175: `BHAGA_ORDER_RECO_ONLY=1` early-exits to `core.order_reco.refresh_order_reco(store)` only (console Option B enqueue). Issue #213: `BHAGA_ADP_SCHEDULE_ONLY=1` early-exits to Team Schedule scrape + BQ `adp_scheduled_shifts`. Issue #267: `BHAGA_ADP_TIMECARD_ONLY=1` early-exits to Timecard scrape + BQ `adp_shifts`/`adp_punches` (no pay_info); Issue #342 adds the Team Schedule to that same login (`download_adp_bundle(include_earnings=False, include_extras=False)`) so open shifts refresh too — a schedule failure only drops `adp_schedule` from the load. Issue #251: `BHAGA_PAYROLL_DRAFT_ONLY=1` early-exits to ADP Start→Preview (leave draft; `BHAGA_ADP_PAYROLL_DRAFT=1` required; optional `BHAGA_PAYROLL_PERIOD_START`/`END` for the console button). Gap compute → scrape → raw → model → reviews → **mechanical + semantic verify** → notify → **record outcome**. After `assert_model_tabs_populated` it runs `model_semantics.assert_model_semantics` (conservation + adp reconciliation + review-bonus survival) and trips the **pipeline halt circuit breaker** on a semantic failure (refuses fresh runs with `EXIT_HALTED` until a healthy run / `--ignore-halt` clears it). The public `main()` is a wrapper around the internal `_run_refresh()`; its `finally` block calls `_record_pipeline_run(run_id=…)` (best-effort, skipped on `--dry-run`) which MERGEs one row into `pipeline_runs` and one row per attempted source into `source_pulls`. **Recorder gate (prod-only):** `_should_record_pipeline_run()` returns True only when `CLOUD_RUN_JOB` env var is set (present in real Cloud Run jobs) or `BHAGA_RECORD_PIPELINE_RUN=1` is explicitly set (cloud-shell backfill opt-in). Laptop and GitHub CI never set `CLOUD_RUN_JOB` and therefore never write to `pipeline_runs` — this keeps Pipeline Health showing prod-only data. **`verify_model_bq` KDS query:** uses `date_local` column (not `date`) when querying `square_kds_daily` for the date range. Powers the "0. Pipeline Health" Grafana section (RUNBOOK §14). **Tests:** `conftest.py` stubs `_record_pipeline_run` for all tests except `test_pipeline_runs_recorder`. CLI: `python3 -m agents.bhaga.scripts.daily_refresh --store palmetto [--date YYYY-MM-DD] [--skip-reviews] [--ignore-halt] [--dry-run]`. |
...
| `payroll_draft_scheduler.py` | **Permanent Monday 07:00 CT** Cloud Scheduler `bhaga-payroll-draft` → `bhaga-daily-refresh:run` with `BHAGA_PAYROLL_DRAFT_ONLY=1`. Does **not** move `bhaga-nightly` (21:30 CT). Spec `build_payroll_draft_job` is pure. |
| `team_pulse.py` | **Team pulse automation** (Issue #216/#233/#245): compose review-bonus leaderboard + motivating template → ClickUp Chat. Fills `{greeting}` (Chicago time of day) + `{pay_cycle}` (current vs dated biweek). Gemini may vary greeting/closers; `accept_varied_copy` rejects multi-draft (`---` / repeated leaderboard). Scheduler: day-gated + soft once-per-CT-date on open period. Console Preview/Post once: Payroll `?period=` filter, **no** once/day cap. Default destination = operator DM; promote via Operator Console. CLI: `--dry-run` / `--once`. Webhook: `POST /team-pulse`. |
...
| `ingest_inventory.py` | **Order Assistant ingest.** Fetches ClickUp closing-form tasks from the "Closing" list, parses per-base quantities via `skills/inventory_parse/parse.py`, and MERGE-upserts into `bhaga.inventory_closing_daily` on natural key `(store, source_task_id, field_id)`. Incremental via `MAX(submitted_ts)` high-water mark; supports `--backfill-from YYYY-MM-DD` and `--date YYYY-MM-DD`. Non-fatal in the nightly — failure leaves today's inventory data stale but does NOT abort the tip/payroll run *only if `_run_refresh()` actually reaches the `run_step` call* (see 2026-07-01 incident below). Grafana sources: `vw_inventory_base_latest_daily` (freshness-checked in GRAFANA_VIEWS) and `vw_inventory_order_assistant` (analytics view — per-base current stock, last-7-eligible-days usage, avg/day, days remaining, restock date; see migration 028). **Since Issue #126, OA numbers live in BQ views** (`vw_order_assistant_table`, `vw_order_reco_combined`); Operator Console `/inventory` is the UI (Issue #276). panel 79 reads `bhaga.vw_order_assistant_table` (analytics table + TOTAL row, `core/migrations/029_order_assistant_functions.sql`). **Since Issue #137, the Order Recommendation is dual-date and MATERIALIZED** (Option D — a live chained slot-2 TVF exceeds BigQuery's query-planner limit): `bhaga.inventory_order_reco` holds one row per (Slot, Item), recomputed by `core/order_reco.refresh_order_reco()` (nightly here in `ingest_inventory.py`'s sibling step + on `/bhaga-cloud restock` submit + on `order_reco_max_tubs` config-set — see `cloud/webhook/handler.py`). Per operator feedback, panel 83 combines both slots into one table via `bhaga.vw_order_reco_combined` (migration 032, a self-join pivot — `Item`/`Current Qty`/`Avg per day` shown once with `frozenColumns` pinning them, each date getting its own `On Hand`/`Order Tubs`/`Order Weight`/`After Restock`/`Days Left`/`Source` columns); the original per-slot views `vw_order_reco_slot1`/`slot2` (migration 031) still exist for freshness tracking but are no longer rendered by any panel. Max Capacity is `store_config.order_reco_max_tubs` (no more live `oa_max_tubs` Grafana textbox); dates come from `vw_order_reco_next_dates` (next 2 future `inventory_restock_schedule` rows). Slot 2 projects as-if slot 1's tubs (estimated or uploaded actual) already arrived — chained off the *materialized* Slot=1 rows, not a nested TVF call. Migration 029's `tvf_order_reco` is superseded but kept as a reference fallback. Both slots show `Order Weight (lbs)` per row (`order_tubs × {Açaí 18 \| other bases 20} lbs/tub`, Blade = NA) and a pallet-aware TOTAL (`+50 lbs` per 40-tub pallet, `CEIL`-rounded), all inside `core/migrations/031_order_reco_dual.sql`'s TVFs. **2026-07-01 incident:** the ingest step's env-building code referenced `run_id` from `main()`'s scope while executing inside the separate `_run_refresh()` function, raising an unhandled `NameError` that aborted the *entire* nightly (not just inventory) the first night this shipped — fixed by threading `run_id` explicitly (`main() → _run_refresh(run_id) → _build_ingest_inventory_env(run_id)`, the last one unit-tested in `test_ingest_inventory_env_wiring.py`). |
...
BigQuery is the **source of truth**. Operator Console reads BQ views. BHAGA Grafana retired Issue #276. Jarvis-dev Grafana still uses `grafana-bq-reader` for PR cost.
