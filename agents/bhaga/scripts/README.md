...
| `otp_gate.py` | OTP **checkpoint-and-resume**: writes a pending request to Firestore + Slack, blocks until the webhook records the operator's reply. **`force_request` flag** (env `BHAGA_OTP_FORCE_REQUEST=1`): when set by an explicit operator trigger (`/bhaga-cloud refresh` or a deploy `Retry-Dates` full-scrape rerun), `evaluate()` treats a stale unanswered checkpoint as a fresh first request — re-saves with a new `requested_at` and re-posts the READY prompt, resetting the 48h window. The nightly path leaves the flag unset (unchanged behaviour). |
...
| `process_reviews.py` | Reviews → date-bracketed bonus allocation ($20 pool for on/after 2026-06-08; legacy $10/$20 per-person before) → rebuild `review_bonus_period`. **`data_window_end`** (the review crediting upper bound) is derived live from `MAX(square_transactions.date_local)` via `core.store_config.resolve_data_window_end()` — never read from `store_config` (a stale stored value would freeze the review window; see 2026-06-15 incident). |
...
| `forecast_ramp_bq.py` | **Parallel adaptive forecast (model: adaptive_dow_ets_v1).** Writes to `model_forecast_ramp_daily` (Section 7A); Section 7 and `forecast_bq.py` are untouched. Holt damped-trend ETS on deseasonalised daily orders × multiplicative DOW factors (rolling 42-day window) × weather/event correction (Ridge on in-sample log-residuals). Agile to plateaus, surges, and dips — static `weeks_since_open` ramp term retired. Public API: `build_ramp_forecast_rows()`, `build_ramp_backfill_rows()`, `build_ramp_coeff_rows()` (adaptive diagnostics: level, trend, damp_sum_30, DOW factors, weather betas → `model_ramp_coeff_daily` for panel 87). Pure-Python Ridge (no numpy/sklearn). `forecast_exclude` applied identically to `forecast_bq`. Skip with `BHAGA_SKIP_FORECAST_RAMP=1`. Research motivation + ramp failure diagnosis: `agents/bhaga/analysis/weather_forecast_spike/`. Prior model `ramp_log_ridge_v1` replaced 2026-06-23 because `weeks_since_open` extrapolated past the Apr–May hypergrowth plateau, causing +120 order bias in Jun 2026. |
...
### Recipe F — add a parallel forecast model (Section 7A pattern)

The Section 7A ramp-aware forecast (`forecast_ramp_bq.py`) is the reference implementation.
To add another parallel model:

1. **Create `forecast_<name>_bq.py`** in `agents/bhaga/scripts/` with public functions
   `build_<name>_forecast_rows(*, labor_daily_rows, weather_rows, horizon_days)` and
   `build_<name>_backfill_rows(...)`. Use `forecast_ramp_bq.py` as the template.
   Set `CURRENT_<NAME>_FORECAST_VERSION` to a unique slug.
2. **Add `map_forecast_<name>_daily(rec)`** in `backfill_bigquery.py` (mirror `map_forecast_ramp_daily`).
3. **Add migration `NNN_<name>_forecast.sql`** in `core/migrations/` — `CREATE TABLE IF NOT EXISTS model_forecast_<name>_daily`, `vw_model_forecast_<name>`, `vw_forecast_<name>_accuracy` (mirror `022_ramp_forecast.sql`).
4. **Wire into `materialize_model_bq.py`** after the ramp block (~line 590), env-gated by `BHAGA_SKIP_FORECAST_<NAME>`, non-fatal.
5. **Add Grafana section** — clone Section 7A panels pointing at the new views.
6. **Tests** mirroring `test_forecast_ramp_bq.py`; **RUNBOOK.md §15A** entry; `PROGRESS.md` dated entry.
