...
7. **Verify the rebuilt Model** — first **mechanically** (`assert_model_tabs_populated`: tabs non-empty,
   ...
   the latest closed period's `adp_paid` reconciles when a covering GCS Earnings export exists, and
   credited review bonuses survived the rebuild). A semantic failure clears the `update_model_sheet`
...
| `model_semantics.py` | **Pure, shared semantic post-conditions** (no I/O): `assert_tip_pool_conserved`, `assert_adp_reconciliation_present` / `assert_period_reconciled`, `assert_review_bonus_present`, and the cadence-gating `assert_model_semantics`. One source of truth used by BOTH `sandbox_e2e` (per-PR gate) and `daily_refresh` (nightly), so a regression can't pass one and fail the other. |
