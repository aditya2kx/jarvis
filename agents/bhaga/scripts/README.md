...
| `forecast.py` | Pure forecast helpers — `_get_parsed_rows` (operating-day parser, honors `forecast_exclude`), plus the legacy DOW-trend/anomaly functions. **The Sheet-writing path (`build_labor_daily_forecast_rows`) is retired.** `forecast_bq.py` only reuses `_get_parsed_rows`. |
| `forecast_bq.py` | **BQ-authoritative daily forecast (simple anchor × growth model, 2026-06-10).** `build_forecast_rows()` returns forward rows `{date, forecast_orders, forecast_items, forecast_generated_at}` for today+1..today+horizon (`forecast_horizon_days` store key, default 30). Each day = the most recent **same-weekday actual** × a **2-week growth multiplier** (`mean(orders, last 7 actual days)/mean(prior 7)`, clamped 0.80–1.20), compounded by the whole weeks between anchor and forecast day. Excluded/closed anchor days are skipped a **whole week at a time** (DOW preserved); items carry the anchor day's actual items × growth. `build_backfill_rows()` writes leakage-free forecasts for the last 8 weeks of PAST dates (each computed using only data before it) so `vw_forecast_accuracy` has forecast-vs-actual history immediately. Called by `materialize_model_bq.py` after the `model_labor_daily` write; loaded via `load_rows("model_forecast_daily", …, merge_keys=["date"])`. Skip with `BHAGA_SKIP_FORECAST=1`. |
