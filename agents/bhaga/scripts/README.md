...
| `team_pulse.py` | **Team pulse automation** (Issue #216/#233): compose review-bonus leaderboard + motivating template → ClickUp Chat. Gemini may vary greeting/closers; `accept_varied_copy` rejects multi-draft (`---` / repeated leaderboard). Day-gated + soft once-per-CT-date. Default destination = operator DM; promote via Operator Console. CLI: `--dry-run` / `--once`. Webhook: `POST /team-pulse`. |
