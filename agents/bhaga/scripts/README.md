...
| `team_pulse.py` | **Team pulse automation** (Issue #216): compose review-bonus leaderboard + motivating template → ClickUp Chat. Day-gated + idempotent per CT date. Default destination = operator DM; promote via Operator Console. CLI: `--dry-run` / `--once`. Webhook: `POST /team-pulse`. |
