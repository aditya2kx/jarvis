...
| `ingest_inventory.py` | **Order Assistant ingest.** Fetches ClickUp closing-form tasks from the "Closing" list, parses per-base quantities via `skills/inventory_parse/parse.py`, and MERGE-upserts into `bhaga.inventory_closing_daily` on natural key `(store, source_task_id, field_id)`. Incremental via `MAX(submitted_ts)` high-water mark; supports `--backfill-from YYYY-MM-DD` and `--date YYYY-MM-DD`. Non-fatal in the nightly — failure leaves today's inventory data stale but does NOT abort the tip/payroll run. |
