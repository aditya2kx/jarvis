...
| `forecast_ramp_bq.py` | **Parallel ramp-aware forecast (model: ramp_log_ridge_v1).** Writes to `model_forecast_ramp_daily` (Section 7A); Section 7 and `forecast_bq.py` are untouched. Log-space Ridge regression refit nightly with features: `weeks_since_open` (ramp term), DOW dummies, lags 7/14/21/28d + 4-week same-DOW rolling mean, weather (`tmean_scaled`, `precip_mm_scaled`, `rainy_flag`). Ridge L2 shrinks weak lag weights automatically. Public API: `build_ramp_forecast_rows()`, `build_ramp_backfill_rows()`, `build_ramp_coeff_rows()` (Ridge β per feature → `model_ramp_coeff_daily` for panel 87 feature-importance chart). Pure-Python Ridge (no numpy/sklearn). `forecast_exclude` applied identically to `forecast_bq`. Skip with `BHAGA_SKIP_FORECAST_RAMP=1`. Research motivation: `agents/bhaga/analysis/weather_forecast_spike/`. |
