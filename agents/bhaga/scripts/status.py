...
GRAFANA_VIEWS: list[Target] = [
    ...
    # migration 027: vw_inventory_base_latest_daily (previously panel 78, 8. Order Assistant timeseries;
    ...
    # migration 043 (Issue #168): plaid_accounts (mask/type for Accounting Account col).
    # Console-only join target — no Grafana panel; no new BQ_TARGETS/GRAFANA_VIEWS.
    ...
    # migration 047 (Issue #189): exclude_from_accounting on taxonomy; Internal
    ...
    # migration 050 (Issue #213): adp_wage_rates.rate_source (earnings | pay_info |
    # roster_stub). Additive column on existing raw table — no new BQ_TARGETS /
    # GRAFANA_VIEWS (wage rates already feed model_labor_daily freshness above).
