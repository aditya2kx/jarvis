...
/**
 * Multi-select + typeahead for in-page tables (not URL-driven).
 * Selection: null = all, [] = none, string[] = partial — same as FilterMultiSelect.
 */
export function LocalMultiSelect({
  ...
  label: string;
  selected: LocalMultiSelection;
  options: string[];
  onChange: (next: LocalMultiSelection) => void;
}) {
  ...
  function push(nextSelected: string[]) {
    if (nextSelected.length === options.length && options.every((o) => nextSelected.includes(o))) {
      onChange(null);
      return;
    }
    ...
    <div className="flex items-center gap-1.5">
      ...
      >
        <Popover.Trigger
          ...
          className={cn(
            "flex h-9 min-h-9 min-w-32 max-w-56 items-center justify-between gap-1 rounded-md border px-2 text-left text-xs font-normal outline-none transition-colors",
            ...
            >
              <div className="flex items-center justify-between gap-2 border-b border-border px-2.5 py-2">
                ...
                <div className="flex items-center gap-2 text-[11px]">
                  <button
                    type="button"
                    className="min-h-9 text-muted-foreground underline-offset-2 hover:text-foreground hover:underline disabled:opacity-40"
                    disabled={isAll}
                    ...
                    return (
                      <li key={opt}>
                        <button
                          ...
                          className={cn(
                            "flex min-h-9 w-full items-start gap-2 rounded-md px-2 py-1.5 text-left text-xs hover:bg-muted",
                            checked && "bg-muted/70",
                          )}
                          onClick={() => {
                            if (isAll) {
