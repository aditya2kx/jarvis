...
describe("triggerModelRecompute", () => {
  ...
  it("POSTs Cloud Run :run exactly once for a multi-date batch", async () => {
    ...
    const out = await triggerModelRecompute([
      "2026-07-06",
      "2026-07-09",
      "2026-07-06",
      "2026-07-08",
    ...
    expect(out.touched).toEqual(["2026-07-06", "2026-07-08", "2026-07-09"]);
    // call 0 = the already-running probe, call 1 = the :run POST
  ...
  it("does not start without calling Cloud Run when dates are empty", async () => {
    expect(await triggerModelRecompute([])).toEqual({ started: false, touched: [] });
    expect(fetch).not.toHaveBeenCalled();
  });

  it("skips :run when a bhaga-daily-refresh execution is already in flight", async () => {
    vi.mocked(fetch).mockResolvedValueOnce({
      ok: true,
      text: async () => "",
      json: async () => ({ executions: [{ name: "x" }] }),
    } as Response);
    const out = await triggerModelRecompute(["2026-07-06", "2026-07-08"]);
    expect(out).toEqual({
      started: false,
