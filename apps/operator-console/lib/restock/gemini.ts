...
// packages with low maintenance signal"). GEMINI_TOKEN comes from Secret
...
export async function parseRestockImage(imageBase64: string, mimeType: string): Promise<ParsedImageRow[]> {
  ...
  if (!geminiToken) throw new Error("parseRestockImage: GEMINI_TOKEN not configured");
  ...
  const res = await fetch(`${GEMINI_URL}?key=${geminiToken}`, {
    method: "POST",
