...
def save_config(overlay: dict) -> bool:
    """Persist the overlay. Returns False when the write did not land.

    Callers must surface a False: an unpersisted radius looks applied until the
    next restart silently reverts it to the geofence.json seed.
    """
    db = _db()
    if db is None:
        log.error("tesla-aladdin-garage fail reason=config_save_no_db")
        return False
    ...
    try:
        ...
        log.error("tesla-aladdin-garage fail reason=config_save err=%r", e)
        return False
