-- 055_order_tub_overrides.sql
-- Issue #225: per-(store, delivery_date, item) manual Order Tubs pins for
-- Estimated reco dates. Actuals (inventory_restock_orders) still win whole-date.
-- Water-fill budget shrinks by SUM(override tubs); overridden items are excluded
-- from candidates and take their pinned qty (including 0).
--
