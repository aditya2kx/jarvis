-- 058_inventory_current_qty_overrides.sql
-- Issue #240: sticky per-(store, item) Current Qty overrides for Order Assistant /
-- Order Reco. COALESCE into vw_inventory_order_assistant.latest_reading so ClickUp
-- ingest keeps writing closings while operator corrections stick until cleared.
--
...
latest_reading AS (
  SELECT
    t.store,
    t.item,
    COALESCE(ov.quantity_units, t.quantity_units) AS current_qty,
    t.submitted_ts AS reported_ts,
    t.submitted_date AS reported_date
  FROM (
    SELECT *,
      ROW_NUMBER() OVER (PARTITION BY store, item ORDER BY submitted_date DESC) AS rn
    FROM base_daily
  ) t
  LEFT JOIN `jarvis-bhaga-prod.bhaga.inventory_current_qty_overrides` ov
    ON ov.store = t.store AND ov.item = t.item
  WHERE t.rn = 1
),
last_restock AS (
