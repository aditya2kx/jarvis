-- 062_payroll_full_roster.sql
-- Issue #251: Payroll roster is everyone ADP will pay this biweek, not only
-- tip-pool PT + excluded FT. Punchers missing from model_tip_alloc_period
-- (Tina, terminated-but-paid Juan, etc.) still get hours/wages rows; tips=0
-- unless they are on tip alloc.
-- Open-period hours window is the ADP biweek through yesterday CT (not the
-- shorter tip-model period_end), so /payroll can match Preview through Aug 22
-- while today is Aug 23.
-- Replaces the 061 view. Same view name — no new Grafana target.
--
...
perks AS (
  SELECT
    ...
    STRING_AGG(
      CONCAT(perk_id, ':', CAST(ROUND(amount_cents / 100.0, 2) AS STRING)),
      ';'
      ORDER BY perk_id
    ) AS perk_reason
