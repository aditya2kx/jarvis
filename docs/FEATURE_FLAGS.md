...
| **Local event auto-open** | `LOCAL_EVENT_AUTO_OPEN=0` to disable | 2026-06 (PR #101) | on | When on (default), a delivered **non-intake** signal opens/focuses the owning worktree's Cursor window. Intake is cloud-primary (Issue #228) — local listener skips worktree creation unless `JARVIS_INTAKE_LOCAL=1`. Set `LOCAL_EVENT_AUTO_OPEN=0` for notify-only. | Keep — window-focus UX, not a temporary cutover. | — |
...
| **Operator Console async order-reco** | `FEATURES.asyncOrderReco` in `apps/operator-console/lib/config/features.ts` | 2026-07 (Issue #175) | **on** | After restock/capacity/self-heal, enqueue `bhaga-daily-refresh` with `BHAGA_ORDER_RECO_ONLY=1` instead of awaiting inline BQ TVFs on the click/RSC path. Brief Order Tubs staleness until the job finishes; UI shows “refreshing…”. Set `false` to restore sync `refreshOrderReco`. | Remove once async path has run stably ≥ 14 days with no operator-reported stale-order incidents. | follow-up |
...
**Cloud-primary intake (Issue #228):** no FEATURE_FLAGS entry — cloud spawn is the default.
Escape hatches (not silent number risk): `new_requirement.py --local`, listener `JARVIS_INTAKE_LOCAL=1`.
Requires GitHub secret `CURSOR_AGENT_TOKEN` (see RUNBOOK §7 Cloud Agent bootstrap).
