...
| **ADP payroll draft** | `FEATURES.adpPayrollDraft` / `BHAGA_ADP_PAYROLL_DRAFT=1` (overrides only) | 2026-08 (Issue #251) | **on** (console) | Playwright Start→Preview→Delete on ADP RUN. Never Approve/Submit. **`bhaga-nightly` 21:30 CT does not Start payroll** (job env unset). Dedicated `bhaga-payroll-draft` scheduler **Monday 07:00 CT** + `/payroll` button for unpaid periods enqueue `BHAGA_PAYROLL_DRAFT_ONLY=1`. Python no-ops mid-biweek Mondays. Next auto: Mon after the next Sunday close. | After ≥1 period-end draft the operator then runs for real, with no leftover In Progress. | follow-up |
