...
| **ADP payroll draft** | `FEATURES.adpPayrollDraft` / `BHAGA_ADP_PAYROLL_DRAFT=1` | 2026-08 (Issue #251) | **off** | Playwright Start→Preview→Delete on ADP RUN. Never Approve/Submit. Nightly hook: `bhaga-nightly` **21:30 CT Monday** after the biweek Sunday (`refresh_date == closed period_end + 1`). This PR leaves Cloud Run env **unset**. | After ≥1 period-end draft the operator then runs for real, with no leftover In Progress. | follow-up |
