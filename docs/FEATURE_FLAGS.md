...
| **Operator Console Accounting** | `FEATURES.accounting` / `FEATURES.writePlaidLink` in `apps/operator-console/lib/config/features.ts` | 2026-07 (Issue #158) | **on** | Gates Accounting nav/page and Plaid Link+sync writes. Code-level flags (not env). Turn off to hide cash ledger UI without undeploying. | Remove once Accounting+Plaid have run stably in prod ≥ 14 days with successful sync screenshots. | follow-up |
