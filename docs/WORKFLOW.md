...
| | Secret scan | verify.py (diff-based; matches assignments, not the words — skips its own definition sites, and treats `KEY=secret-name:latest` as a Secret Manager version reference rather than a value, so Cloud Run env edits don't trip the password pattern — #316) | gate | M2 | agent | M2 ✓ (#285) |
