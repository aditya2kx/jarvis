...
| **Labor** | Square from `vw_model_labor_daily`; labor $ / % `vw_labor_daily_live`; `adp_scheduled_shifts` (actual vs schedule, concurrent, coverage); `adp_open_shifts` (open / unassigned slots — hatched violet stack on upcoming Hours-chart days except Hour/Weekday, dashed lanes in coverage incl. past gaps; Issue #342); Sync scheduled → `BHAGA_ADP_SCHEDULE_ONLY`; Sync clocked hours → `BHAGA_ADP_TIMECARD_ONLY` (Timecard + Team Schedule) | Sync scheduled/clocked; weekly hours goal → `store_config.goal_labor_hours_week` (same as Home) |
...
- **Scheduled vs actual** cut at yesterday CT; Sync scheduled shifts runs `BHAGA_ADP_SCHEDULE_ONLY` (local or Cloud Run) → purge-before-upsert into `adp_scheduled_shifts`. **Sync clocked hours** (Labor + Payroll) runs `BHAGA_ADP_TIMECARD_ONLY` → Timecard XLSX + `adp_shifts`/`adp_punches`, plus the Team Schedule in the same login so `adp_open_shifts` refreshes (no pay_info, no liability). **Open shifts** (Issue #342): `laborOpenShiftHoursByGrain` (schedule window, gate `showChartOpenShifts`) feeds the `open` series (`Series.pattern: "hatch"` in `BarChartCard`); tooltip adds Open (unassigned) + Total if filled, which the weekly Goal compares against. `laborOpenShiftDays` (whole chart window) feeds `LaborCoveragePanel` open lanes (`buildOpenLanesForDate` greedy packing; chip `+N open`; headcount stays people-only). `adpScheduleHorizonEnd` also considers `adp_open_shifts`. Target date = past coverage chip or closed period end, else yesterday CT. Horizon: up to **8** forward weeks (stop when the Team Schedule › chevron does not advance); draft weeks are included when ADP shows them in the same grid (Issue #230).
