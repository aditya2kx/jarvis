...
| 2026-07-04 | Auth (revised) | Native Cloud Run IAM (`--no-allow-unauthenticated` + `roles/run.invoker`), not Google IAP as decided 2026-07-02 — `gcloud run deploy --iap` failed in CI because creating the IAP OAuth brand requires a Google Workspace organization, which `jarvis-bhaga-prod` does not have; the legacy `gcloud iap oauth-brands`/`oauth-clients` commands are also deprecated project-wide. A custom-OAuth-client "External"-audience IAP path may exist without a Workspace org, but re-opening that is a follow-up, not a blocker for this PR. `lib/auth/identity.ts::operatorEmail()` now decodes the caller's Authorization Bearer ID token's `email` claim (Cloud Run's IAM check already verified it) instead of relying on an IAP-forwarded header; operators reach the service via `gcloud run services proxy` (see `RUNBOOK.md` §17) instead of a plain browser URL |
| 2026-07-05 | Identity header (revised) | `gcloud run services proxy` sends the caller's verified ID token in `X-Serverless-Authorization`, not the plain `Authorization` header assumed on 2026-07-04 — confirmed live via a `DEBUG_AUTH_HEADERS` header dump. `operatorEmail()` now checks `X-Serverless-Authorization` first, falls back to `Authorization: Bearer`, and as a last resort calls Google's `tokeninfo` endpoint for callers presenting an opaque OAuth2 access token instead of a JWT (no decodable payload) |
| 2026-07-05 | Caching (security fix) | Every page switched from `export const revalidate = 600` to `export const dynamic = "force-dynamic"` — discovered while re-locking the preview after the temporary public-access review window that Next's Full Route Cache had cached `/home` (and other pages) at the CDN edge while public, and kept serving that cached 200 to new unauthenticated callers even after IAM was re-enabled, because the cache lookup happens before Cloud Run's IAM check can matter for a *cached* response. `force-dynamic` guarantees every request is server-rendered fresh, so IAM is enforced on every hit |
| 2026-07-05 | Admin allowlist (interim) | `operatorEmail()` now accepts an identity if it is inside `@mypalmetto.co` **or** listed in the `ALLOWED_EMAILS` allowlist. This lets a personal Google account (e.g. an owner's gmail) be granted admin access for pre-merge testing without weakening the domain rule for everyone else. The allowlist value lives in Secret Manager (`operator-console-allowed-emails`), wired via `--set-secrets ALLOWED_EMAILS=…` in the deploy workflow — kept out of git so no personal address is committed, and changeable without a rebuild. Reaching the service still requires a `roles/run.invoker` binding per account. This is a stopgap: it is superseded once the Google Groups + IAP (`admin`/`operator` groups) model lands as a follow-up |
| 2026-07-05 | Auth (reversal of 2026-07-04 pivot) | **Cloud Run direct IAP**, replacing the `gcloud run services proxy` + Bearer-token + `ALLOWED_EMAILS` model from the two rows above. The 2026-07-04 pivot hit the Google-managed/internal OAuth brand flow (`gcloud iap oauth-brands`), which requires a Workspace org `jarvis-bhaga-prod` doesn't have; current GCP docs confirm a custom **"External"**-audience OAuth client — created once via the Google Auth Platform Console page + the Cloud Run Security tab's IAP checkbox, no `gcloud iap oauth-brands` involved — explicitly supports "projects that aren't in a Google Cloud organization." The operator now opens the plain `https://…run.app` URL and signs in with Google directly; access is gated by `roles/iap.httpsResourceAccessor` IAM (per-user now, Google Groups later) with no app-level allowlist. `lib/auth/identity.ts::operatorEmail()` resolves the email from `X-Goog-Authenticated-User-Email` and cross-verifies the signed `X-Goog-IAP-JWT-Assertion` via `google-auth-library` against the direct-Cloud-Run-IAP audience format. The `operator-console-allowed-emails` secret and the two `user:* run.invoker` bindings from the interim model are deleted. Folded into PR #147, blocking its merge until verified end-to-end (browser sign-in, deny, write attribution) |
| 2026-07-12 | Home + Accounting (Issue #158) | Home renamed **Goal and Tracking**: net sales, PT labor %, total labor %, prep p95@8, Risky base count=0. Plaid Link + `/transactions/sync` ledger (migration 037) + `/accounting` (Square in / Plaid out, PFC interim). Custom categorization + QBO deferred to follow-up issues. |
| 2026-07-26 | Auth (OAuth Audience publish) | Google Auth Platform Audience was left at **Testing** after the 2026-07-05 External OAuth setup. That forced short-lived grants + frequent re-consent; on iPhone Chrome the consent leg (`/signin/oauth/consent`) intermittently 500'd after idle until a manual refresh. Published to **In production** via Console (Playwright-driven; no org API). IAM (`roles/iap.httpsResourceAccessor`) still gates access — publish does not widen who can open the console. Keep Data Access on non-sensitive scopes only so verification stays unnecessary. See `RUNBOOK.md` §17 step 3. |
| 2026-07-27 | Payroll recognition + Lindsay IAP (#206) | `vw_model_payroll_period` (migration 049) joins `recognition_bonuses` into est_total_pay / bonus_diff; Payroll UI: recognition columns + headline, drawer period/employee picker, tip-exemption sticky headers + Date/Employee filters; IAP grant for `lindsay@mypalmetto.co` in deploy loop. |
| 2026-07-28 | Auth (single-host login, #208) | Drop post-IAP hash→canonical middleware bounce. Cross-host 302 forced a second OAuth onto a host with no `__Host-GCP_IAP_*` cookies (mobile Error code 9 / account-picker loops). Sole operator URL = project-number canonical host; hash form unsupported; `$0` infra. |
| 2026-07-28 | Auth (replace IAP with Auth.js, #210) | Cloud Run browser IAP still drove iOS Google `signin/oauth/consent` + `rapt=` 401 after #208. Replace with Auth.js Google OAuth + `ALLOWED_EMAILS`; deploy `--allow-unauthenticated --no-iap`; same canonical URL; `$0` infra. |
