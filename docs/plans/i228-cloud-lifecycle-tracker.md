...
| C3 | `phase_state` init / advance from Cloud VM | **partial** — GH SoT exists; cache seed is laptop/`--local` only | MED | **FAIL** | `phase_state.py status` → `Issue: #none`, 0% done, despite #228 labels `approved:jam` / `approved:define-evidence`. Cache not hydrated. | no | **Build:** `phase_state` hydrate-from-GH on cloud boot / `init --issue 228` in spawn seed; verify `status` shows #228 |
...
| C9 | `verify.py` (+ pytest) in Cloud VM | **partial** — script present; env incomplete | MED | **FAIL** | `verify.py --fast` → **pytest-changed FAIL** (`No module named pytest`). Hooks path set after manual `install-git-hooks.sh`. | no | **Build:** ensure `requirements.txt` / env install includes pytest; re-run `--fast` then `--full` |
