...
def _make_in_memory_store_patchers():
    ...
    def _find_duplicate_sessions(pr, session_keys):
        my_key = S.pr_key(pr)
        result = []
        for key, rec in db.items():
            if key == my_key:
                continue
            for s in (rec.get("build") or {}).get("sessions") or []:
                uid = S._session_uid(s)
                if uid in session_keys:
                    result.append((key, uid))
        return result
    ...
    patcher = patch.multiple(
        ...
        find_duplicate_sessions=_find_duplicate_sessions,
    )
    return patcher, db
