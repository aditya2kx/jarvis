#!/usr/bin/env python3
"""ADP People → Payroll info → Hourly pay rate gap-fill (Issue #213).

Earnings & Hours only has rates after a paycheck. New punchers (e.g. Brooke /
Elizabeth) appear on timecards but not on earnings — labor $ then falls back
to median. This backend reads the setup rate from Payroll info and upserts
``adp_wage_rates`` with ``rate_source=pay_info``. Earnings Regular always
wins when both exist (loaded separately; this path only fills NULL gaps).
"""
...
def scrape_one_pay_info(page, canonical_name: str, *, dashboard_url: str) -> dict:
    ...
    # Wait for People directory search (not "Search Shortcuts" omnibox).
    search = page.locator(
    ...
    try:
        ...
        # Directory may use a different label — take first visible Search input
        # that is not Shortcuts/omnisearch.
        search = page.locator(
            'input[placeholder*="Search" i]:not([data-test-id="omnisearch-input"])'
            ':not([placeholder*="Shortcuts" i])'
        ).first
        search.wait_for(state="visible", timeout=20_000)
    ...
    inputs = page.evaluate(
        """() => {
          const out = [];
          for (const el of document.querySelectorAll('input, sdf-input')) {
...
def write_pay_info_json(rates: list[dict], *, store: str = "palmetto") -> pathlib.Path:
...
def gap_names_from_session_files(
    *,
    ...
    names: set[str] = set()
    if timecard_xlsx and timecard_xlsx.exists():
        punches = sb.parse_xlsx(timecard_xlsx, employee_aliases=employee_aliases)
        names = {p["employee_name"] for p in punches if p.get("employee_name")}

    return sorted(names - have)


def write_pay_info_rates_bq(rates: list[dict], *, dry_run: bool = False) -> int:
    """MERGE pay_info rates only for employees still missing a non-null rate."""
    ...
    ensure_schema()
    have = names_with_nonnull_rates_bq()
    fill = [r for r in rates if r["employee_name"] not in have and r.get("wage_rate_dollars")]
    skipped = len(rates) - len(fill)
    if skipped:
        print(f"[pay_info] skip {skipped} already-rated employee(s) (earnings wins)")
    if not fill:
        return 0
    profile = load_store_profile("palmetto")
    bq_rows = [map_adp_wage_rate(r, profile) for r in fill]
    if dry_run:
        print(f"[pay_info] DRY: would MERGE {len(bq_rows)} rows")
        for r in fill:
    ...
    print(f"[pay_info] adp_wage_rates MERGE {n} rows (rate_source=pay_info)")
    return n
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    ap.add_argument("--from-bq-gaps", action="store_true", help="Resolve gaps from BQ")
    ...
    with launch_persistent(portal="adp", headed=args.headed, slow_mo_ms=50) as (_ctx, page):
        ...
        rates = scrape_pay_info_rates(page, names, dashboard_url=dashboard_url)
        write_pay_info_json(rates, store=args.store)
        if args.write_bq or args.dry_run:
            write_pay_info_rates_bq(rates, dry_run=args.dry_run)
        if args.write_bq and not args.dry_run:
            assert_no_missing_puncher_rates(days=args.days)
    return 0
